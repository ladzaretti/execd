...
if [[ "$OS" != linux ]]; then
    echo "unsupported operating system: $OS (the installer requires Linux and systemd)" >&2
    exit 1
fi
...
if [[ "$VERSION" == latest ]]; then
    VERSION=$(curl -s https://api.github.com/repos/ladzaretti/execd/releases/latest |
        grep '"tag_name":' |
        sed -E 's/.*"v([^\"]+)".*/\1/')
fi
...
TARBALL="execd_${VERSION}_${OS}_${ARCH}.tar.gz"
DEST="$TMP_DIR/$TARBALL"
URL="https://github.com/ladzaretti/execd/releases/download/v$VERSION/$TARBALL"
...
cd "$TMP_DIR/execd_${VERSION}_${OS}_${ARCH}"
